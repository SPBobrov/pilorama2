...
ТОЛЩИНА <-34
ШИРИНА <- 127
ДАТА_НАЧАЛО <- "26.10.2021"
ДАТА_КОНЕЦ <- "10.11.2021"

ТАБЛИЦА_СКЛАДА_10055 <- subset(x = ТАБЛИЦА_СКЛАДА, subset = Номер == '10055') # сортировать по номеру
View(ТАБЛИЦА_СКЛАДА_10055)

ТАБЛИЦА_СКЛАДА_34_127 = subset(x = ТАБЛИЦА_СКЛАДА, subset = толщина...9 == ТОЛЩИНА&ширина == ШИРИНА) # сортировать по типоразмеру
# просмотр полученной таблицы
View(ТАБЛИЦА_СКЛАДА_34_127)

ТАБЛИЦА_СКЛАДА_34_127 <- transform (ТАБЛИЦА_СКЛАДА_34_127, 'Дата погрузки' = as.Date('Дата погрузки', format = "%d.%m.%Y")) # трансформируем дату в формат даты
# просмотр полученной таблицы
View(ТАБЛИЦА_СКЛАДА_34_127)

ТАБЛИЦА_СКЛАДА_34_127_DATE <- subset(x = ТАБЛИЦА_СКЛАДА_34_127, subset = 'Дата погрузки' >= ДАТА_НАЧАЛО&'Дата погрузки' <=ДАТА_КОНЕЦ)

# просмотр полученной таблицы
View(ТАБЛИЦА_СКЛАДА_34_127_DATE)
